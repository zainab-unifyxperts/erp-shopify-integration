...
* Ubuntu v22.04
* ERPNext v15
* [Marketplace](https://github.com/raymond-fung/erp-marketplace)
* Install the ShopifyAPI `bench pip install --upgrade ShopifyAPI`
  
### Installation Guide
Prequisites: 
* Get the app from github and install it on your site.
   **Note** Ensure that the read permission is granted to the user cloning the github repo
* If not do the following:
  1) Login in to the github account that has access to the current repository
  2) Settings -> SSH and GPG keys -> new SSH Key -> add the ssh key of the machine you're installing this frappe app on 
* `bench get-app git@github.com:raymond-fung/erp-shopify.git` - Downloads an app from a git repository 
* `bench install-app shopify_integration` - Installs Shopify Integration on current site
...
3) Setup Shopify shop for GraphQL
   * The sales orders won't start syncing unitl you setup your shopify shop to send and receive information via GraphQL 
...
4) Customizing Cron
   * Currently the cron is setup to sync orders every 3 minutues
   * You can enter a valid cron string in `scheduler_events = {
    "cron": {
        "*/3 * * * *": [
            "shopify_integration.shopify_selling.orders.shopify_order_sync_job"
        ]
    },
}` which can be found in `shopify_integration/hooks.py`
* **Note run `bench migrate` everytime you install a new app now your site or make changes to hooks.py**  
